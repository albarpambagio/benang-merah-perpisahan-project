...
PDF_DIR = "pdfs_async"
MAX_PAGES = 499  # Target all 499 pages
INITIAL_CONCURRENT_WORKERS = 12  # Initial concurrency
PAGE_BATCH_SIZE = 25  # Process pages in batches
REQUEST_TIMEOUT = 60.0
POLITENESS_DELAY = (1.5, 3.0)
BATCH_SIZE = 20  # Increased batch size
CHECKPOINT_FILE = 'scraper_checkpoint.txt'
MAX_RETRIES = 3
RETRY_DELAYS = [3, 7, 15]  # Progressive delays for retries
MAX_CONCURRENT_PDF_DOWNLOADS = 5  # Separate limit for PDF downloads
MAX_PENDING_TASKS = 50  # Limit for task queue
MEMORY_THRESHOLD = 80  # Percentage at which to reduce workload
        ...
        if table:
            ...
            for row in table.xpath('.//tr'):
                ...
                if label == 'Nomor':
                    ...
                    item['catatan_amar'] = tds[1].text_content(" ", strip=True)
        ...
        for page_url in page_batch:
            try:
                ...
                for url in filtered_case_urls:
                    ...
                    while task_queue.qsize() >= stats.current_concurrency:
                        await asyncio.sleep(0.1)
                    
                    task = asyncio.create_task(
                        worker(url, client, lock, stats, batch, batch_lock, pdf_semaphore, existing_case_numbers)
                    )
                    await task_queue.put(task)
                
                # Process with progress bar
                pbar = tqdm(total=len(filtered_case_urls), desc=f"Processing {len(filtered_case_urls)} cases")
