...
class CourtCasesSpiderMiddleware:
...
class CourtCasesDownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
    ...
    def process_request(self, request, spider):
        if self.circuit_open:
            if time.time() - self.last_failure_time > 60:
                ...
                raise IgnoreRequest("Circuit breaker open")
        return None


class PDFDelayMiddleware:
    def process_request(self, request, spider):
        if request.url.lower().endswith('.pdf'):
            request.meta['download_timeout'] = 120  # Special timeout for PDFs
        return None
